...
- **Power & Electrical**: live voltage, current (mA), computed wattage, connected adapter wattage, and an animated flow diagram between adapter/battery/Mac. While actively charging, shows a 3-way split (adapter → battery, real, computed from V×I; adapter → Mac, **estimated** as adapter's rated wattage minus battery draw). This estimate is only accurate when the Mac is pulling the adapter's full rated capacity — true for small/low-wattage adapters under load (verified: with a 15W adapter while charging, this gave 7.0W to battery / ~8W to Mac, matching Juicy's own displayed split almost exactly), false for a high-wattage adapter at light load (would overstate Mac draw by the unused headroom). Labeled as an estimate in the UI, not presented as a true independent measurement — there's no public API for that.
