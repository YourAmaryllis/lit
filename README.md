...
- **Devices**: Bluetooth/HID peripheral battery (AirPods, Beats, Magic Mouse/Keyboard/Trackpad, anything else exposing standard HID battery), still unverified against real hardware — no accessory has been paired on this Mac during testing. Also **iPhone/iPad battery percentage** via `libimobiledevice` (LGPL-2.1, invoked as an external subprocess — `brew install libimobiledevice` — never linked into the app, so this stays MIT-clean). Researched first: there's no reliable Bluetooth-only API for this — the real mechanism (confirmed from `AirBattery`'s own source, which is AGPL-3.0 and therefore not something we could vendor) is USB-pair-once then Wi-Fi sync over the private `lockdownd` protocol. The "no device" path is verified (confirmed empty/clean, no crash); the actual battery-reading path is **unverified against a real device** — see `mac-app/DATA_SOURCES.md` §5 for exactly what to check if it doesn't work.
