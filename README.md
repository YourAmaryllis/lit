...
- **Devices**: Bluetooth/HID peripheral battery (AirPods, Beats, Magic Mouse/Keyboard/Trackpad, anything else exposing standard HID battery), still unverified against real hardware — no accessory has been paired on this Mac during testing. Also **iPhone/iPad battery percentage + charging state**, with a proper 📱/`ipad`/`iphone` icon, via `libimobiledevice` (LGPL-2.1, invoked as an external subprocess — `brew install libimobiledevice` — never linked into the app, so this stays MIT-clean). Researched first: there's no reliable Bluetooth-only API for this — the real mechanism (confirmed from `AirBattery`'s own source, which is AGPL-3.0 and therefore not something we could vendor) is USB-pair-once then Wi-Fi sync over the private `lockdownd` protocol. **Verified end-to-end against a real iPad**: hit Apple's USB Restricted Mode first (device must be unlocked within the last hour for USB data access, not just cable-connected), then confirmed real live data flowing through — see `mac-app/DATA_SOURCES.md` §5 for the full story and what to check if it doesn't work for you. Cycle count/health isn't exposed for iOS devices this way — only percentage and charging state are real, available data.
...
- **System Temperature**: real CPU/GPU temperature via the SMC (System Management Controller) — the same undocumented mechanism smcFanControl/TG Pro/iStat Menus and open-source `exelban/stats` use. Struct layout and Apple Silicon sensor key codes were researched and ported from `stats`' real source before writing anything (a wrong memory layout on a raw IOKit call risks a hang/crash, not just wrong data). Key list is **model-specific and only verified for this machine** (Mac16,12, base M4) — other chip generations will simply show no CPU/GPU temperature, never wrong data. Plus `ProcessInfo.thermalState` (public API) as a qualitative Nominal/Fair/Serious/Critical badge that works on any Mac. **Hit and fixed a genuinely tricky bug here**: a `.load(as: UInt32.self)` call on an unaligned `[UInt8]` buffer was undefined behavior, producing different garbage on every single run (not a consistent wrong answer) — which looked exactly like a connection/timing/permissions problem and cost real time to correctly diagnose before landing on the actual cause. Full story in `mac-app/DATA_SOURCES.md` §7. Verified with 3 consecutive fresh app launches post-fix, all correct immediately.
...
- iPhone/iPad full battery **health** (cycle count, health %, temperature — not just the percentage we now show) — the same `libimobiledevice`/`com.apple.mobile.battery` domain we use for percentage likely has more of this, but hasn't been tried
